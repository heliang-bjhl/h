...
seajs.use(["$",'select',"textLimit", "confirmbox", "util"], function($, Select, textLimit, ConfirmBox, util) {
    $(function() {
        ...
        insertTable.bind({
            triggerSelector : '#jAddMarketTr',
            tBody : '#jtbMarket tbody',
            contextSelector : '#jAddMarketLab'
        });
        insertTable.bind({
            triggerSelector : '#jAddMerchantTr',
            tBody : '#jtbMerchant tbody',
            contextSelector : '#jAddMerchantLab'
        });
        //dialog显示内容
        var html = [
        ...
        //编辑类型
        $('.jLeaveBtn').on('click','.j-change-mes',function(){
            dialog.show();
            var $this = $(this);
